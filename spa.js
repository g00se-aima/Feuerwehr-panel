...
function renderHauptwachePage() {
  ...
  // Lager Hauptwache button removed - it's now on the dedicated Lager page
  ...
  'lager': {
    ...
    content: `<div class="card center-col">
      ...
      <div class="page-buttons" style="margin-bottom:18px">
        ...
        <button class="btn btn-grey" style="font-size:1.1rem;min-width:180px;padding:12px;border:none" onclick="navigate('lager-hauptwache')">Lager Hauptwache (Container)</button>
        <button class="btn btn-black" style="font-size:1.1rem;min-width:180px;padding:12px;border:none" onclick="navigate('AGW')">Geräte Verwalten</button>
      </div>
...
function renderPersonalMasksOverview() {
  ...
  try { moveables = JSON.parse(localStorage.getItem(moveablesKeyFor(pageKey)) || '[]'); } catch (e) { moveables = []; }
    ...
    if (personal.length) {
      ...
      wrap.style.gap = '6px';
      // Sort personal masks numerically by AM number
      const sortedPersonal = personal.sort((a, b) => {
        const numA = parseInt((a.label || '').replace(/^AM\s+/, '').split(/\s+/)[0], 10);
        const numB = parseInt((b.label || '').replace(/^AM\s+/, '').split(/\s+/)[0], 10);
        if (isNaN(numA) || isNaN(numB)) return ((a.label || '') || '').localeCompare((b.label || '') || '', 'de');
        return numA - numB;
      });
      sortedPersonal.forEach(m => {
        const chip = document.createElement('span');
    ...
    main.innerHTML = `
      <div class="card center-col">
        ...
        <div class="main-grid" style="margin-bottom:18px">
          <button class="btn btn-grey" onclick="navigate('lager')">Lager / Silschede</button>
...
function renderVehiclePage(page) {
  ...
  const btn = document.createElement('button');
    ...
    if (typeof window.makeButtonMoveable === 'function') {
      ...
      updateClickToRemoveState(btn);
      // For all vehicle/storage pages, add click handler to open assignment sidebar
      if (typeof showAssignmentSidebar === 'function') {
